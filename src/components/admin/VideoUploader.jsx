...
import { Upload, Check, AlertCircle, Loader2, X, Video } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

export default function VideoUploader({ folder, initialUrl, onUploadComplete, label }) {
  ...
  const handleFileSelection = (e) => {
    ...
    if (!file.type.startsWith('video/')) {
      setError('Please select a video file (MP4, WEBM, OGG, etc.)');
      return;
    }

    // Validate size (limit to 100MB for safe uploads in browser context)
    ...
    // Create local object URL for preview before upload
  ...
  return (
    <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col gap-3">
      <div className="flex justify-between items-center">
        <label className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">{label || 'Upload Video'}</label>
      ...
      <div className="relative w-full aspect-[16/9] rounded-lg bg-slate-900/60 border border-dashed border-slate-800 overflow-hidden flex items-center justify-center group">
        {previewUrl ? (
          ...
          <div className="flex flex-col items-center gap-1.5 text-slate-600">
            ...
            <span className="text-[9px] uppercase font-bold tracking-wider">No Video Preview</span>
          </div>
        )}
      ...
      <div className="flex flex-col gap-2">
        ...
        {success && !selectedFile && (
          <div className="flex items-center gap-1.5 text-emerald-500 text-[10px] font-bold">
            ...
            <span>Video Ready & Verified</span>
          </div>
        )}

        {!selectedFile && !error && !success && (
          <span className="text-[9px] text-slate-500 font-semibold uppercase tracking-wider">
            Supports MP4, WEBM, MKV, etc. (Max 100MB)
          </span>
        )}
      </div>
    </div>
