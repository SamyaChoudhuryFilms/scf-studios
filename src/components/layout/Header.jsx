...
export default function Header() {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 w-full transition-all duration-300 px-4 md:px-12 py-4 flex items-center justify-between ${mobileMenuOpen ? 'z-[150]' : 'z-50'
        ...
        <div className="flex items-center gap-3 md:gap-8">
          ...
          <Link to="/" className="flex items-center gap-3 text-2xl font-black tracking-wider text-brand-accent hover:opacity-90 select-none">
            <img src="/logo-square.jpg" className="h-8 w-8 object-contain invert brightness-150 contrast-125 rounded-md" alt="SCF Logo" />
            <span>SCF STUDIOS</span>
          </Link>
            ...
            {navLinks.map(link => {
              ...
              return (
                <Link
                  key={link.to}
                  to={link.to}
                  className={`text-sm font-semibold tracking-wide transition-colors duration-200 ${isActive ? 'text-brand-accent font-bold' : 'text-text-secondary hover:text-text-primary'
                    }`}
                >
                  {link.label}
                </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <form onSubmit={handleSearchSubmit} className="relative hidden md:block" ref={searchRef}>
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute right-0 mt-2 w-72 bg-card-bg/50 backdrop-blur-[5px] border border-white/10 rounded-lg shadow-2xl p-2 z-50">
          ...
          <div className="relative hidden md:block" ref={notificationRef}>
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-3 w-80 bg-card-bg/50 backdrop-blur-[5px] border border-white/10 rounded-lg shadow-2xl overflow-hidden z-50 animate-slide-in">
          ...
          {activeProfile ? (
            <div className="relative" ref={dropdownRef}>
              ...
              {profileDropdownOpen && (
                <div className="absolute right-0 mt-3 w-56 bg-card-bg/50 backdrop-blur-[5px] border border-white/10 rounded-lg shadow-2xl overflow-hidden z-50">
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 z-[160] md:hidden bg-black/75 animate-fade-in"
          onClick={() => setMobileMenuOpen(false)}
        />
      )}
      <div
        className={`fixed top-0 left-0 h-full w-[280px] z-[170] bg-bg-secondary border-r border-white/5 shadow-2xl transition-transform duration-300 md:hidden flex flex-col ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
          <span className="flex items-center gap-2.5 text-lg font-black tracking-wider text-brand-accent select-none">
            <img src="/logo-square.jpg" className="h-6.5 w-6.5 object-contain invert brightness-150 contrast-125 rounded-md" alt="SCF Logo" />
            <span>SCF STUDIOS</span>
          </span>
          <button
            onClick={() => setMobileMenuOpen(false)}
          ...
          {[
            ...
            return (
              <Link
                ...
                to={link.to}
                onClick={() => setMobileMenuOpen(false)}
                className={`text-base font-bold tracking-wide py-2 border-b border-white/[0.02] flex items-center transition-colors ${isActive ? 'text-brand-accent font-bold' : 'text-text-secondary hover:text-text-primary'
                  }`}
              >
                {link.label}
              </Link>
